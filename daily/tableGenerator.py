class TableGenerator:
    ...
    def previousDayTransformation(day, silver_path):
        ...
        decripted_table = decrypt_data(silver_table, encription_key)
        
        ingest_table = decripted_table.withColumn("Ticker_decripted", decrypt_udf(col('Ticker'), lit(encription_key.encode('utf-8'))))
        ingest_table = ingest_table.drop("Ticker").withColumnRenamed("Ticker_decripted", "Ticker")
        ingest_table = ingest_table.filter(col("Date")>day)
